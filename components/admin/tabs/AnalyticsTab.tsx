...
export const AnalyticsTab: React.FC<Props> = ({ products, orders, siteEvents, range, onRangeChange }) => {
  ...
  const renderLocationRankList = (
    ...
    <div className="rounded-xl border border-white/10 bg-slate-900 p-5 shadow-sm border-white/10 bg-slate-900">
      <div className="mb-4">
          ...
          return (
            <div key={row.label}>
              <div className="mb-1 flex items-center justify-between gap-3 text-sm">
                <span className="truncate font-semibold text-white" title={row.label}>{row.label}</span>
                <span className="shrink-0 text-xs text-slate-400">{row.visitors} visitors</span>
              </div>
              <div className="h-2 overflow-hidden rounded-full bg-white/10 bg-white/10">
                <div className="h-full rounded-full bg-primary-500" style={{ width: `${width}%` }} />
              </div>
              <p className="mt-1 text-[11px] text-slate-400">{row.events} events</p>
            </div>
          );
        })}
        {rows.length === 0 && (
          <p className="rounded-lg border border-dashed border-white/15 p-4 text-sm text-slate-400 border-white/10">
    ...
    <div className="space-y-8 animate-fade-in-up">
        ...
        right={
          <div className="flex flex-wrap items-center gap-2">
            <select
              ...
              className="h-9 rounded-lg border border-white/15 bg-slate-900 px-3 text-sm text-white bg-slate-800 text-white border-white/15"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-slate-900 p-6 rounded-xl border border-white/5 shadow-sm">
          ...
          <p className="text-3xl font-bold text-white mt-2">{summary.totalOrders}</p>
        </div>
        <div className="bg-slate-900 p-6 rounded-xl border border-white/5 shadow-sm">
        ...
        <div className="bg-slate-900 p-6 rounded-xl border border-white/5 shadow-sm">
          ...
          <p className="text-3xl font-bold text-white mt-2">{summary.totalCustomers}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-slate-900 p-6 rounded-xl border border-white/5 shadow-sm">
          <p className="text-sm text-slate-400 uppercase tracking-wide">Page Views</p>
          <p className="text-3xl font-bold text-white mt-2">{siteSummary.pageViews}</p>
          <p className="mt-2 text-xs text-slate-400">Website visits tracked in selected range</p>
        </div>
        <div className="bg-slate-900 p-6 rounded-xl border border-white/5 shadow-sm">
          <p className="text-sm text-slate-400 uppercase tracking-wide">Unique Visitors</p>
          <p className="text-3xl font-bold text-white mt-2">{siteSummary.uniqueVisitors}</p>
          <p className="mt-2 text-xs text-slate-400">Based on browser sessions</p>
        </div>
        <div className="bg-slate-900 p-6 rounded-xl border border-white/5 shadow-sm">
          <p className="text-sm text-slate-400 uppercase tracking-wide">Add To Cart</p>
          <p className="text-3xl font-bold text-white mt-2">{siteSummary.actionCounts.addToCart}</p>
          <p className="mt-2 text-xs text-slate-400">Product add events</p>
        </div>
        <div className="bg-slate-900 p-6 rounded-xl border border-white/5 shadow-sm">
          <p className="text-sm text-slate-400 uppercase tracking-wide">Checkout Actions</p>
          <p className="text-3xl font-bold text-white mt-2">{siteSummary.actionCounts.beginCheckout}</p>
          <p className="mt-2 text-xs text-slate-400">Checkout started events</p>
        </div>
      </div>

      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 shadow-sm border-white/10 bg-white/[0.03] sm:p-6">
        <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="text-xl font-bold text-white">Location Analytics</h2>
        ...
        <div className="mb-5 rounded-xl border border-white/10 bg-slate-900 p-4 border-white/10 bg-slate-900">
          <div className="mb-3">
            <p className="text-sm font-semibold text-white">Filters</p>
            <p className="text-xs text-slate-400">Refine visitor events by date, location, event type, or product.</p>
          </div>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-7">
            <label className="text-xs font-semibold text-slate-400">
              ...
              <input
                ...
                className="mt-1 h-10 w-full rounded-lg border border-white/15 bg-slate-900 px-3 text-sm text-white border-white/15 bg-slate-800 text-white"
              />
            </label>
            <label className="text-xs font-semibold text-slate-400">
              Date to
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-6">
          {[
            ...
            <div key={label} className="rounded-xl border border-white/10 bg-slate-900 p-4 border-white/10 bg-slate-900">
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-2">
          <div className="rounded-xl border border-white/10 bg-slate-900 p-5 shadow-sm border-white/10 bg-slate-900">
            <h3 className="text-base font-semibold text-white">Activity by Location</h3>
            <p className="mb-4 text-xs text-slate-400">Location-wise event counts for pages, product views, cart activity, checkouts, purchases, and logins.</p>
            <div className="space-y-3">
              {locationSummary.activityByLocation.map((location) => (
                <div key={location.key} className="rounded-lg border border-white/10 p-4 border-white/10">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold text-white">{location.label}</p>
                      <p className="text-xs text-slate-400">{location.events} events - {location.visitors} visitors</p>
                    </div>
                    <p className="text-xs text-slate-400">{new Date(location.latest).toLocaleString()}</p>
                  </div>
                  <div className="mt-3 grid grid-cols-3 gap-2 text-xs sm:grid-cols-6">
                    {[
                      ...
                      <div key={label} className="rounded-md bg-gray-50 px-2 py-2 text-slate-300 bg-white/5 text-slate-300">
          ...
          <div className="rounded-xl border border-white/10 bg-slate-900 p-5 shadow-sm border-white/10 bg-slate-900">
            <h3 className="text-base font-semibold text-white">Add to Cart Activity by Location</h3>
            <p className="mb-4 text-xs text-slate-400">Product-wise cart activity grouped by location.</p>
            <div className="space-y-3">
              {locationSummary.addToCartByLocation.map((item) => (
                <div key={item.key} className="rounded-lg border border-white/10 p-4 border-white/10">
                  <p className="line-clamp-2 font-semibold text-white">{item.productName}</p>
        ...
        <div className="mt-5 rounded-xl border border-white/10 bg-slate-900 p-5 shadow-sm border-white/10 bg-slate-900">
          <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
            <div>
              <h3 className="text-base font-semibold text-white">Recent Visitors</h3>
              <p className="text-xs text-slate-400">Latest filtered activity across pages, product views, carts, checkout, and login.</p>
            </div>
            <p className="text-xs font-semibold text-slate-400">{locationSummary.totalVisitors} visitors - {locationSummary.totalEvents} events</p>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-[1280px] text-sm">
              <thead className="bg-white/5">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left text-slate-400">Event</th>
                  <th className="px-3 py-2 text-left text-slate-400">Visitor / Session</th>
                  <th className="px-3 py-2 text-left text-slate-400">Location</th>
                  <th className="px-3 py-2 text-left text-slate-400">Page</th>
                  <th className="px-3 py-2 text-left text-slate-400">Product</th>
                  <th className="px-3 py-2 text-left text-slate-400">Qty</th>
                  <th className="px-3 py-2 text-left text-slate-400">Price</th>
                  <th className="px-3 py-2 text-left text-slate-400">Device</th>
                </tr>
              </thead>
              <tbody>
                {locationSummary.recentEvents.map((event, index) => (
                  <tr key={event.id || `${event.timestamp}-${index}`} className={index % 2 === 0 ? 'bg-slate-900' : 'bg-white/5'}>
                    <td className="whitespace-nowrap px-3 py-2 text-slate-300">{new Date(event.timestamp).toLocaleString()}</td>
                    <td className="px-3 py-2 font-semibold text-white">{formatEventName(isCheckoutEvent(event.event) ? 'checkout' : event.event)}</td>
                    <td className="max-w-[210px] px-3 py-2 text-slate-300">
                      <p className="truncate" title={eventVisitorKey(event)}>{eventVisitorKey(event)}</p>
                      <p className="truncate text-[11px] text-slate-400" title={event.sessionId}>{event.sessionId || '-'}</p>
                    </td>
                    ...
                    <td className="max-w-[220px] truncate px-3 py-2 text-slate-300" title={event.pageLocation || event.pagePath || '-'}>
                      {event.pagePath || event.pageLocation || '-'}
                    </td>
                    <td className="max-w-[220px] truncate px-3 py-2 text-slate-300" title={event.productName || '-'}>
                      {event.productName || '-'}
                    </td>
            ...
            {locationSummary.recentEvents.length === 0 && (
              <p className="mt-4 rounded-lg border border-dashed border-white/15 p-4 text-sm text-slate-400 border-white/10">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-slate-900 p-6 rounded-xl border border-white/10">
          <h3 className="text-lg font-semibold text-white mb-1">Most Visited Pages</h3>
          <p className="text-xs text-slate-400 mb-4">Similar to Vercel/Firebase page analytics, grouped by page URL.</p>
          <div className="space-y-3">
            {siteSummary.topPages.map((page) => (
              <div key={page.path} className="rounded-lg border border-white/10 bg-gray-50 p-3 border-white/10 bg-white/5">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-white">{page.title || page.path}</p>
                    <p className="truncate text-xs text-slate-400">{page.path}</p>
                  </div>
                  <div className="shrink-0 text-right">
                    <p className="text-sm font-bold text-white">{page.views}</p>
                    <p className="text-[11px] text-slate-400">{page.visitors} visitors</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-slate-900 p-6 rounded-xl border border-white/10">
          <h3 className="text-lg font-semibold text-white mb-1">Customer Actions</h3>
          <p className="text-xs text-slate-400 mb-4">Tracks important shopping actions from the website.</p>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={label} className="rounded-lg bg-gray-50 p-3 bg-white/5">
          ...
          <h4 className="mt-6 text-sm font-semibold text-white">Most Added Products</h4>
          <div className="mt-3 space-y-2">
            {siteSummary.topAddToCartProducts.map((product) => (
              <div key={product.id} className="flex items-center justify-between gap-3 rounded-lg border border-white/10 px-3 py-2 border-white/10">
                <div className="min-w-0">
      ...
      <div className="bg-slate-900 p-6 rounded-xl border border-white/10">
        <h3 className="text-lg font-semibold text-white mb-1">Traffic Source</h3>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-white/5">
              <tr>
                <th className="px-3 py-2 text-left text-slate-400">Source</th>
                <th className="px-3 py-2 text-left text-slate-400">Visitors</th>
                <th className="px-3 py-2 text-left text-slate-400">Page Views</th>
                <th className="px-3 py-2 text-left text-slate-400">Add To Cart</th>
                <th className="px-3 py-2 text-left text-slate-400">Purchases</th>
              </tr>
            </thead>
            <tbody>
              {siteSummary.trafficSources.map((source, idx) => (
                <tr key={source.source} className={idx % 2 === 0 ? 'bg-slate-900' : 'bg-white/5'}>
                  <td className="px-3 py-2 font-semibold text-white">{source.source}</td>
                  <td className="px-3 py-2 text-slate-300">{source.visitors}</td>
                  <td className="px-3 py-2 text-slate-300">{source.pageViews}</td>
                  <td className="px-3 py-2 text-slate-300">{source.addToCart}</td>
                  <td className="px-3 py-2 text-slate-300">{source.purchases}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="bg-slate-900 p-6 rounded-xl border border-white/10">
        <h3 className="text-lg font-semibold text-white mb-1">Visitor Details</h3>
        <p className="text-xs text-slate-400 mb-4">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[1260px] text-sm">
            <thead className="bg-white/5">
              <tr>
                <th className="px-3 py-2 text-left text-slate-400">Last Seen</th>
                <th className="px-3 py-2 text-left text-slate-400">Source</th>
                <th className="px-3 py-2 text-left text-slate-400">Referrer</th>
                <th className="px-3 py-2 text-left text-slate-400">IP</th>
                <th className="px-3 py-2 text-left text-slate-400">Location</th>
                <th className="px-3 py-2 text-left text-slate-400">Pincode</th>
                <th className="px-3 py-2 text-left text-slate-400">Geo</th>
                <th className="px-3 py-2 text-left text-slate-400">Login</th>
                <th className="px-3 py-2 text-left text-slate-400">Phone</th>
                <th className="px-3 py-2 text-left text-slate-400">Page Views</th>
                <th className="px-3 py-2 text-left text-slate-400">Add To Cart</th>
                <th className="px-3 py-2 text-left text-slate-400">Purchases</th>
                <th className="px-3 py-2 text-left text-slate-400">Last Page</th>
              </tr>
            </thead>
            <tbody>
              {siteSummary.visitorDetails.map((visitor, idx) => (
                <tr key={visitor.sessionId} className={idx % 2 === 0 ? 'bg-slate-900' : 'bg-white/5'}>
                  <td className="px-3 py-2 text-slate-300">{new Date(visitor.lastSeen).toLocaleString()}</td>
                  <td className="px-3 py-2 font-semibold text-white">{visitor.source}</td>
                  <td className="max-w-[180px] truncate px-3 py-2 text-slate-300" title={visitor.referrer || '-'}>
                    {visitor.referrer || '-'}
                  </td>
                  <td className="px-3 py-2 text-slate-300">{visitor.ipAddress || 'Not captured yet'}</td>
                  <td className="max-w-[180px] truncate px-3 py-2 text-slate-300" title={visitor.location || 'Not detected'}>
                    {visitor.location || 'Not detected'}
                  </td>
                  <td className="px-3 py-2 text-slate-300">{visitor.pincode || '-'}</td>
                  <td className="px-3 py-2 text-slate-300">{formatLocationSource(visitor.locationSource || undefined)}</td>
                  <td className="px-3 py-2 text-slate-300">{visitor.isLoggedIn ? 'Logged in' : 'Guest'}</td>
                  <td className="px-3 py-2 text-slate-300">{cleanPhone(visitor.phone)}</td>
                  <td className="px-3 py-2 text-slate-300">{visitor.pageViews}</td>
                  <td className="px-3 py-2 text-slate-300">{visitor.addToCart}</td>
                  <td className="px-3 py-2 text-slate-300">{visitor.purchases}</td>
                  <td className="max-w-[180px] truncate px-3 py-2 text-slate-300" title={visitor.lastPage}>
                    {visitor.lastPage}
                  </td>
                </tr>
              ))}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 bg-slate-900 p-6 rounded-xl border border-white/10">
          <h3 className="text-lg font-semibold text-white mb-1">Revenue by Month ({selectedYear})</h3>
          <p className="text-xs text-slate-400 mb-3">Higher bar = higher revenue in that month.</p>
          ...
          {!hasMonthlyRevenue && (
            <p className="mb-3 rounded-lg border border-dashed border-white/15 bg-gray-50 p-3 text-sm text-slate-400 border-white/10 bg-white/5">
            ...
            <div className="grid h-56 min-w-[760px] grid-cols-12 items-end gap-3">
              {monthly.map((m) => (
                <div key={m.label} className="flex min-w-0 flex-col items-center gap-2">
                  <div className="flex h-40 w-full items-end rounded-md bg-white/10 bg-white/10">
                    <div
        ...
        <div className="bg-slate-900 p-6 rounded-xl border border-white/10">
          <h3 className="text-lg font-semibold text-white mb-1">Category Share</h3>
          <p className="text-xs text-slate-400 mb-3">Shows which category contributes most revenue.</p>
          ...
          <div className="mt-4 space-y-2">
            ...
            {Object.keys(summary.categoryRevenue).length === 0 && (
              <p className="text-sm text-slate-400">No category data in selected range.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900 p-6 rounded-xl border border-white/10">
        <h3 className="text-lg font-semibold text-white mb-1">Orders by Month ({selectedYear})</h3>
        <p className="text-xs text-slate-400 mb-3">Line up means more orders, line down means fewer orders.</p>
        <div className="flex items-center gap-2 mb-4 text-xs text-slate-300">
          <span className="inline-block w-3 h-3 rounded bg-emerald-500" />
          <span>Green line: Number of orders</span>
        </div>
        <div className="rounded-lg border border-white/10 p-3 overflow-x-auto">
            ...
            {monthly.map((m, idx) => {
              ...
              return (
                <g key={`dot-${m.label}`}>
                  ...
                  <text x={x} y={Math.max(8, y) - 8} textAnchor="middle" fontSize="10" fill="currentColor" className="text-slate-300">
                    {m.orders}
                  </text>
                  <text x={x} y={176} textAnchor="middle" fontSize="11" fill="currentColor" className="text-slate-400">
                    {m.label}
                  </text>
                </g>
              );
      ...
      <div className="bg-slate-900 p-6 rounded-xl border border-white/10">
        <h3 className="text-lg font-semibold text-white mb-4">Simple Monthly Summary ({selectedYear})</h3>
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-white/5">
              <tr>
                <th className="px-3 py-2 text-left text-slate-400">Month</th>
                <th className="px-3 py-2 text-left text-slate-400">Revenue</th>
                <th className="px-3 py-2 text-left text-slate-400">Orders</th>
              </tr>
            </thead>
            <tbody>
              {monthly.map((m, idx) => (
