...
const WarrantyRegistration = React.lazy(() => import('./pages/WarrantyRegistration').then(module => ({ default: module.WarrantyRegistration })));
const Complaint = React.lazy(() => import('./pages/Complaint').then(module => ({ default: module.Complaint })));
...
const PrivacyPolicySection = React.lazy(() => import('./pages/PrivacyPolicySection').then(module => ({ default: module.PrivacyPolicySection })));
...
const getRouteSeo = (pathname: string) => {
 ...
 description: 'Shop smart bands, smart rings, smart fans and monitoring wearables from TheFutureX.',
    },
    '/smart-bands': {
      title: 'Smart Bands',
 ...
 description: 'Discover TheFutureX smart monitoring devices for wellness and daily care.',
    ...
    '/gifting-store': {
      title: 'Gifts',
      description: 'Shop premium TheFutureX technology gifts for wellness, style, fitness, and smarter homes.',
    },
    '/shop': {
      title: 'Shop All Products',
 description: 'Browse all TheFutureX smart wearables, smart fans and monitoring products.',
    },
    '/shop/all': {
      title: 'Shop All Products',
 description: 'Browse all TheFutureX smart wearables, smart fans and monitoring products.',
    ...
    '/verify-phone': {
      title: 'Verify Phone',
      description: 'Verify your phone number to continue your TheFutureX order.',
      robots: 'noindex, follow',
    },
    '/payment': {
      title: 'Payment',
      description: 'Complete payment for your TheFutureX order.',
      robots: 'noindex, follow',
    },
    '/order-success': {
      title: 'Order Success',
      description: 'Your TheFutureX order confirmation.',
      robots: 'noindex, follow',
    },
    '/track-order': {
      title: 'Track Your Order',
      description: 'Track your TheFutureX order status.',
      robots: 'noindex, follow',
    },
    '/profile': {
      title: 'Profile',
      description: 'Manage your TheFutureX account profile.',
      robots: 'noindex, follow',
    },
    '/login': {
      title: 'Login',
    ...
    '/register-warranty': {
      ...
      description: 'Register your TheFutureX product and claim 1 month extra warranty on eligible purchases.',
      robots: 'noindex, follow',
    },
    '/raise-complaint': {
      title: 'Give Feedback',
      description: 'Share TheFutureX product, order, delivery, warranty, or support feedback with proof images.',
      robots: 'noindex, follow',
    },
 '/privacy-policy-section': {
 title: 'Privacy Policy Section',
 description: 'Wellness data disclaimer for TheFutureX app and wearable integrations.',
      robots: 'noindex, follow',
    },
    '/admin': {
      title: 'Admin Dashboard',
      description: 'TheFutureX admin dashboard.',
      robots: 'noindex, nofollow',
    },
    '/admin/edit-product': {
      title: 'Edit Product',
      description: 'TheFutureX admin product editor.',
      robots: 'noindex, nofollow',
    },
  };
...
const collectionSchemaByPath: Record<string, { name: string; description: string }> = {
  '/smart-bands': {
    name: 'Smart Bands',
 description: 'Explore TheFutureX smart bands for activity tracking, everyday insights and connected living.',
  },
...
const RouteSeo: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const collection = collectionSchemaByPath[cleanPath];
    if (collection) {
      setCollectionPageJsonLd({
        path: cleanPath,
...
const LegacyProductRedirect: React.FC = () => {
  const { pathname } = useLocation();
  const slug = pathname.split('/').filter(Boolean).pop() || '';
  return <Navigate to={slug ? `/product/${slug}` : '/shop/all'} replace />;
};
...
const App: React.FC = () => {
  useEffect(() => {
    return runAfterPageSettles(preloadVisitorGeo, 2500);
  }, []);
        ...
        <CartProvider>
          <BrowserRouter>
            <AuthModalProvider>
              ...
              <RouteAnalytics />
              <OrderSourceTracker />
              <div className="flex flex-col min-h-screen text-gray-100 bg-dark-bg transition-colors duration-300 relative overflow-x-hidden">
                <Header />
                <CartDrawer />
                <main className="site-main-content flex-grow">
                  <Suspense fallback={<LoadingFallback />}>
                    <Routes>
                      ...
                      <Route path="/product-category/smart-ring" element={<Navigate to="/smart-rings" replace />} />
                      <Route path="/product-category/smart-rings" element={<Navigate to="/smart-rings" replace />} />
                      <Route path="/product-category/smart-fan" element={<Navigate to="/bladeless-fan" replace />} />
                      <Route path="/product-category/smart-fans" element={<Navigate to="/bladeless-fan" replace />} />
                      <Route path="/product-category/monitor" element={<Navigate to="/smart-monitoring" replace />} />
                        <Route path="/product-category/smart-monitoring" element={<Navigate to="/smart-monitoring" replace />} />
                      <Route path="/gifting-store" element={<GiftingStore />} />
                      <Route path="/gifting-store/:giftMode" element={<GiftingStore />} />
                      ...
                      <Route path="/shop/:category/:slug" element={<LegacyProductRedirect />} />
