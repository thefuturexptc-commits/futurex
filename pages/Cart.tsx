    ...
    <div className="min-h-screen max-w-7xl mx-auto px-4 py-10 sm:py-12 pb-24 sm:pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          {items.map((item) => (
            <div key={item.id} className="rounded-xl border border-white/10 bg-white/5 p-3.5 sm:p-4 shadow-sm flex items-center space-x-3 sm:space-x-4">
              ...
              <div className="flex flex-col items-end space-y-2">
                <div className="flex items-center space-x-2 bg-white/10 rounded-lg p-1">
                  <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-8 h-8 flex items-center justify-center text-white hover:bg-white/10 rounded shadow-sm">-</button>
      ...
      <div className="sm:hidden fixed left-0 right-0 bottom-0 z-40 border-t border-white/10 bg-dark-bg/95 backdrop-blur px-4 py-3">
        <div className="flex items-center justify-between text-sm mb-2 text-gray-200">
          <span>Total</span>
          <span className="font-bold text-white">₹{(totalPrice * 1.08).toFixed(2)}</span>
        </div>
