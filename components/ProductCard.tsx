  ...
  return (
    ...
    >
      ...
      {!product.isBestSeller && product.isNewArrival && (
        <div className="product-new-arrival-badge absolute right-3 top-3 z-20 rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.16em] shadow-[0_10px_22px_-14px_rgba(34,184,180,0.9)]">
          New Arrival
        </div>
      )}
      <Link to={`/product/${toProductSlug(product.name)}`} className={`product-card-media relative flex items-center justify-center overflow-hidden bg-transparent ${imageAspectClassName || (compact ? 'aspect-[4/3]' : 'aspect-[4/5]')}`}>
        <img
          src={activeImage}
          alt={product.name}
          loading="lazy"
          ...
          width={640}
          height={800}
          className={`h-full w-full ${imageFit === 'cover' ? 'object-cover' : 'object-contain'} object-center transition-all duration-300 ease-out ${enableHoverEffects ? 'group-hover:scale-[1.025]' : ''
            }`}
        />
        <div
      ...
      <div className={`${compact ? 'p-3 sm:p-3.5' : 'p-4 sm:p-6'} flex flex-col flex-1`}>
        ...
        <div className={compact ? 'mt-auto pt-2 space-y-2.5' : 'flex flex-col gap-2 mt-auto pt-3 sm:pt-5'}>
          <div className={`flex flex-col ${compact ? '' : ''}`}>
            <span className={`tfx-price-pill w-fit px-3 py-1.5 ${compact ? 'text-sm sm:text-base' : 'text-base sm:text-lg'} font-black font-display leading-none`}>Rs {salePrice}</span>
            <div className="flex items-center gap-1.5">
              <span className={`${compact ? 'text-[9px] sm:text-[10px]' : 'text-[10px] sm:text-xs'} line-through ${mutedTextClass}`}>Rs {mrp}</span>
              {percent > 0 && <span className="text-xs font-semibold text-slate-500">{percent}% off</span>}
            </div>
          ...
          <Button
            ...
            className={
              compact
                ? 'tfx-buy-now-cta h-9 w-full rounded-full px-4 text-xs font-black shadow-sm sm:h-10'
                : 'tfx-buy-now-cta h-10 w-full rounded-full px-4 text-xs font-black shadow-sm'
            }
          >
