...
const TFX5_SMART_BAND_SPECS: Record<string, string> = {
  ...
  'Charging Instructions': 'Device: place it in the charging slot at the bottom of the Battery Pack to start charging - the green light flashes slowly while charging and stays on solid once full; tap the button to wake the device from sleep mode. Battery Pack: insert a Type-C cable to start charging (about 90 minutes for a full charge) - the green light stays on for 3 seconds then turns off when full; double tap the top of the Battery Pack to wake it from sleep. The Battery Pack indicator also shows the device battery level - red flashing for 0-25%, yellow flashing for 25-80%, green flashing for 80-100% - and flashes red 3 times before shutting down below 5%. Supports about 7+ days of normal use and 20 days of standby, depending on usage.',
  'Operating Temperature': 'Operate between 32°F to 113°F (0°C to 45°C) and store between -4°F to 158°F (-20°C to 70°C). Using or storing the device outside these ranges can damage it and shorten battery life; avoid sudden changes in temperature or humidity.',
  'Safety Instructions': 'Not a medical device - do not use as a substitute for professional medical advice. Handle with care: avoid dropping, burning, puncturing, or crushing the device, keep it away from heavy dust or sand, do not use if damaged, and supervise children around it. Do not attempt to replace the battery yourself; the lithium-ion battery should only be serviced by the manufacturer or an authorized provider, must not be incinerated, and should be recycled separately rather than thrown in household waste. Avoid extreme temperatures or direct sunlight, and never sleep on the device or cover it with a blanket, pillow, or your body while it is charging or connected to power. If you experience skin irritation, itching, or any allergic reaction, stop wearing the device immediately and seek medical help right away if a severe reaction occurs.',
};
