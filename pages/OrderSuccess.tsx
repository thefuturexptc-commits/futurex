...
export const OrderSuccess: React.FC = () => {
  ...
  const orderDetailsHref = orderId !== 'Unknown'
    ...
    : '/track-order';
  const warrantyRegistrationHref = orderId !== 'Unknown'
    ? `/register-warranty?orderId=${encodeURIComponent(orderId)}`
    : '/register-warranty';
  ...
  return (
    <div className="order-success-page min-h-screen bg-[#f5fbfb] px-4 py-10 text-slate-950 sm:py-14">
      <div className="mx-auto flex min-h-[calc(100vh-7rem)] w-full max-w-xl items-center justify-center">
        ...
        <div className="order-success-warranty-offer mb-7 rounded-2xl border border-teal-200 bg-teal-50 p-4 text-left">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-teal-700">Warranty Offer</p>
          <h2 className="mt-2 text-lg font-bold text-slate-950">Claim 1 month extra warranty</h2>
          <p className="mt-1 text-sm leading-6 text-slate-600">
            Register your product now with your purchase invoice to activate the extra warranty benefit.
          </p>
          <Link
            to={warrantyRegistrationHref}
            className="mt-4 inline-flex h-11 w-full items-center justify-center rounded-lg bg-teal-600 px-5 text-sm font-bold text-white transition-colors hover:bg-teal-700"
          >
